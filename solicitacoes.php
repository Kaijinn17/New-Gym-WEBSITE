  ...
  <center>
    ...
    if ($sql->rowCount() > 0) {
      ...
      foreach ($sql->fetchAll(PDO::FETCH_ASSOC) as $dado) {
        ...
        echo "<td><a href='./src/aprovar.php?id=" . $dado['id'] . '&telefone=' . $dado['telefone'] . '&planos=' . $dado['plano_novo'] . "' class='btn btn-success btn-sm' style='margin-right:10px;' name='update' id='update'>APROVAR</a><a href='./src/reprovar.php?id=" . $dado['id'] . "' class='btn btn-danger btn-sm'>REPROVAR</a></td>";
        echo "</tr>"; // fecha linha
      }
